<?php
// Template name: Blastoff
?>
<?php get_header();?>
  <main class="main overflow-hidden">
    <section class="section-project light-orbs">
      <div class="container">
        <div class="text-center">
          <p class="fs-headline-lg color-magenta fw-bold mb-1 mb-md-3">O programa</p>
          <h2 class="fs-display-md text-white text-uppercase fw-medium m-0">BlastOff</h2>
        </div>
        <figure class="rocket-animation d-flex align-items-center justify-content-center text-center w-100">
          <img class="w-100 object-fit-contain" src="<?php echo get_stylesheet_directory_uri();?>/public/assets/gifs/rocket_animation.gif" alt="rocker">
        </figure>
        <div>
          <p class="scroll-down-text text-center text-white text-uppercase mb-4">
            Scroll down
          </p>
          <figure class="scroll-down-button">
            <img class="w-100" src="<?php echo get_stylesheet_directory_uri();?>/public/assets/icons/arrow-bottom-scrolldown.svg" alt="down">
          </figure>
        </div>
      </div>
    </section>
    <section class="section-information">
      <div class="container d-flex align-items-end align-items-md-center justify-content-between gap-4">
        <div class="text-white w-50">
          <h2 class="text-container fs-display-lg mb-3">
            trabalhamos <br> hoje mirando o <span class="color-magenta">amanhã</span>
          </h2>
          <p class="text-container fs-title-lg fw-bold">
            E foi com esse pensamento que nasceu o programa <span class="color-light-blue">BlastOff</span> que tem como objetivo a preparação de jovens para serem os mais novos programadores e desenvolvedores no furturo! Quer saber mais? Acompanhe!
          </p>
        </div>
        <figure class="information-rocket m-0">
          <img class="w-100" src="<?php echo get_stylesheet_directory_uri();?>/public/assets/images/rocket.svg" alt="">
        </figure>
      </div>
    </section>
    <section class="section-about light-orbs">
      <div class="container">
        <div class="text-center">
          <p class="fs-headline-lg color-magenta fw-bold mb-1 mb-md-3">Sobre</p>
          <h2 class="fs-display-md text-white fw-medium m-0">Como funciona?</h2>
        </div>
        <div>
          <ul class="about-list text-white list-unstyled">
            <li class="about-list-item bg-blur">
              <p class="fs-title-lg fw-bold mb-2">
                Desafios
              </p>
              <p class="about-item-text">
                O Blastoff aborda todos os desafios de treinamento e desenvolvimento de maneira personalizada e escalável.
              </p>
            </li>
            <li class="about-list-item bg-blur">
              <p class="fs-title-lg fw-bold mb-2">
                Ensino
              </p>
              <p class="about-item-text">
                O programa é projetado para preencher o gap que existe entre o ensino tradicional e o mercado de trabalho.
              </p>
            </li>
            <li class="about-list-item bg-blur">
              <p class="fs-title-lg fw-bold mb-2">
                Habilidades
              </p>
              <p class="about-item-text">
                Nós Desenvolvemos as competências e habilidades dos colaboradores criando um ambiente saudável, produtivo, motivado e competitivo.
              </p>
            </li>
            <li class="about-list-item bg-blur">
              <p class="fs-title-lg fw-bold mb-2">
                Objetivos
              </p>
              <p class="about-item-text">
                Colaboramos para encontrar e reter talentos para atingir os objetivos estratégicos da organização.
              </p>
            </li>

          </ul>
        </div>
        <div class="text-center text-white">
          <figure class="m-0">
            <img src="<?php echo get_stylesheet_directory_uri();?>/public/assets/icons/rocket-icon.svg" alt="">
          </figure>
          <p class="text-box">
            Em um cenário de alta demanda por profissionais com <span class="color-light-blue">habilidades técnicas</span> em desenvolvimento de software e escassez de talentos, impulsionamos organizações de diferentes tamanhos com profissionais qualificados para desenvolvimento de <span class="color-light-blue">front end e back end</span> para atuarem no mercado e vencerem desafios.</p>
        </div>
      </div>
    </section>
    <section class="section-professionals-title">
      <div class="container">
        <div class="text-center">
          <p class="fs-headline-lg color-magenta fw-bold mb-1 mb-md-3">Profissionais</p>
          <h2 class="fs-display-md text-white fw-medium m-0">Conheça os instrutores responsáveis</h2>
        </div>
      </div>
    </section>
    <section class="section-professionals">
      <div class="container">
        <div>
          <ul class="professional-list list-unstyled text-white text-center">
            <li class="professional-list-item bg-blur">
              <figure class="profile-image">
                <img class="w-100" src="<?php echo get_stylesheet_directory_uri();?>/public/assets/images/pamela.png" alt="">
              </figure>
              <div class="d-flex flex-column h-100">
                <h3 class="mt-4 mb-3">Pamela Andrelo</h3>
                <p>
                  Mestre em Informática e Gestão do Conhecimento. “Quando todos avançam juntos, o sucesso ocorre por si só.” — Henry Ford
                </p>
              </div>

            </li>
            <li class="professional-list-item bg-blur">
              <figure class="profile-image">
                <img class="w-100" src="<?php echo get_stylesheet_directory_uri();?>/public/assets/images/flavio.png" alt="">
              </figure>
              <h3 class="fs-title-lg mt-4 mb-3">Flavio Bergamini</h3>
              <p class="">
                Formado em Engenharia da Computação. Possuo o objetivo de entregar soluções com qualidade, e a cada dia aprender cada vez mais.
              </p>
            </li>
          </ul>
        </div>
      </div>
    </section>
    <section class="section-apprenticeship light-orbs">
      <div class="container light-orbs">
        <div class="text-white d-flex flex-sm-row justify-content-between align-items-sm-end mb-sm-5 flex-column align-items-center text-center text-sm-start">
          <div class="">
            <p class="fs-headline-lg color-magenta fw-bold mb-1 mb-md-3">Fundamentos e Soft skills</p>
            <h2 class="fs-display-md text-white fw-medium m-0">O que você <br> pode aprender?</h2>
          </div>
          <div class="fs-title-lg">
            <p class="apprenticeship-info text-sm-end mb-4 mt-2 m-sm-0">
              O aluno pode aprender a <span class="color-light-blue fw-bold">programar</span>, aprendendo desde lógica de programação até uma linguagem e ou framework específico.
            </p>
          </div>
        </div>
        <div>
          <ul class="apprenticeship-list list-unstyled">
            <li class="apprenticeship-button previous bg-magenta cursor-pointer">
            </li>
            <li class="apprenticeship-button next bg-magenta cursor-pointer">

            </li>
            <li class="apprenticeship-list-item bg-blur text-white active" data-list-item="0">
              <h2 class="apprenticeship-item-title">01</h2>
              <h3 class="apprenticeship-item-subtitle">Lógica de programação</h3>
              <div class="apprenticeship-elements-hidden">
                <p class="apprenticeship-item-info fs-title-lg">
                  Lógica de programação é a técnica de encadear pensamentos para atingir determinado objetivo. Estes pensamentos, podem ser descritos como uma seqüência de Lógica de programação é a técnica de encadear pensamentos para atingir determinado objetivo.
                </p>
                <a class="toodoo-button text-decoration-none fw-bold" href="#">Saiba mais</a>
              </div>
            </li>
            <li class="apprenticeship-list-item bg-blur text-white" data-list-item="1">
              <h2 class="apprenticeship-item-title">02</h2>
              <h3 class="apprenticeship-item-subtitle">Banco de Dados</h3>
              <div class="apprenticeship-elements-hidden">
                <p class="apprenticeship-item-info fs-title-lg">
                  Lógica de programação é a técnica de encadear pensamentos para atingir determinado objetivo. Estes pensamentos, podem ser descritos como uma seqüência de...
                </p>
                <a class="toodoo-button text-decoration-none fw-bold" href="#">Saiba mais</a>
              </div>
            </li>
            <li class="apprenticeship-list-item bg-blur text-white" data-list-item="2">
              <h2 class="apprenticeship-item-title">03</h2>
              <h3 class="apprenticeship-item-subtitle">Repositório e Versionador de Código</h3>
              <div class="apprenticeship-elements-hidden">
                <p class="apprenticeship-item-info fs-title-lg">
                  Lógica de programação é a técnica de encadear pensamentos para atingir determinado objetivo. Estes pensamentos, podem ser descritos como uma seqüência de...
                </p>
                <a class="toodoo-button text-decoration-none fw-bold" href="#">Saiba mais</a>
              </div>
            </li>
            <li class="apprenticeship-list-item bg-blur text-white" data-list-item="3">
              <h2 class="apprenticeship-item-title">04</h2>
              <h3 class="apprenticeship-item-subtitle">Programação Orientada a Objetos</h3>
              <div class="apprenticeship-elements-hidden">
                <p class="apprenticeship-item-info fs-title-lg">
                  Lógica de programação é a técnica de encadear pensamentos para atingir determinado objetivo. Estes pensamentos, podem ser descritos como uma seqüência de...
                </p>
                <a class="toodoo-button text-decoration-none fw-bold" href="#">Saiba mais</a>
              </div>
            </li>
          </ul>
          <ul class="apprenticeship-tip-list list-unstyled">
            <li class="apprenticeship-tip-item active" data-list-item="0"></li>
            <li class="apprenticeship-tip-item" data-list-item="1"></li>
            <li class="apprenticeship-tip-item" data-list-item="2"></li>
            <li class="apprenticeship-tip-item" data-list-item="3"></li>
          </ul>
        </div>
      </div>
    </section>
    <section class="section-process">
      <div class="container">
        <div class="text-center">
          <p class="fs-headline-lg color-magenta fw-bold mb-1 mb-md-3">O processo seletivo</p>
          <p class="fs-title-lg text-white">
            Entenda como funciona nosso <br> processo seletivo
          </p>
        </div>
        <div>
          <ul class="process-primary-list list-unstyled text-white">
            <li class="process-primary-item">
              <ul class="process-secondary-list list-unstyled">
                <li class="process-secondary-item">
                  <figure class="process-secondary-image">
                    <img class="w-100" src="<?php echo get_stylesheet_directory_uri();?>/public/assets/icons/process-registrations.svg" alt="">
                  </figure>
                  <h3 class="process-secondary-title">Inscrições</h3>
                  <p class="process-secondary-text">As inscrições serão feitas através do site da toodoo.</p>
                </li>
                <li class="process-secondary-item">
                  <figure class="process-secondary-image">
                    <img class="w-100" src="<?php echo get_stylesheet_directory_uri();?>/public/assets/icons/process-participate.svg" alt="">
                  </figure>
                  <h3 class="process-secondary-title">Quem pode participar?</h3>
                  <p class="process-secondary-text">
                    Todas as pessoas acima de 18 anos com muita vontade de aprender e se tornarem dev’s.
                  </p>
                </li>
                <li class="process-secondary-item">
                  <figure class="process-secondary-image">
                    <img class="w-100" src="<?php echo get_stylesheet_directory_uri();?>/public/assets/icons/process-exam.svg" alt="">
                  </figure>
                  <h3 class="process-secondary-title">Prova técnica</h3>
                  <p class="process-secondary-text">
                    O prazo para responder a prova técnica será sempre de 24h.s
                  </p>
                </li>
              </ul>
            </li>
            <li class="process-primary-item">
              <ul class="process-secondary-list list-unstyled">
                <li class="process-secondary-item">
                  <figure class="process-secondary-image">
                    <img class="w-100" src="<?php echo get_stylesheet_directory_uri();?>/public/assets/icons/process-registrations.svg" alt="">
                  </figure>
                  <h3 class="process-secondary-title">Inscrições</h3>
                  <p class="process-secondary-text">As inscrições serão feitas através do site da toodoo.</p>
                </li>
                <li class="process-secondary-item">
                  <figure class="process-secondary-image">
                    <img class="w-100" src="<?php echo get_stylesheet_directory_uri();?>/public/assets/icons/process-participate.svg" alt="">
                  </figure>
                  <h3 class="process-secondary-title">Quem pode participar?</h3>
                  <p class="process-secondary-text">
                    Todas as pessoas acima de 18 anos com muita vontade de aprender e se tornarem dev’s.
                  </p>
                </li>
                <li class="process-secondary-item">
                  <figure class="process-secondary-image">
                    <img class="w-100" src="<?php echo get_stylesheet_directory_uri();?>/public/assets/icons/process-exam.svg" alt="">
                  </figure>
                  <h3 class="process-secondary-title">Prova técnica</h3>
                  <p class="process-secondary-text">
                    O prazo para responder a prova técnica será sempre de 24h.s
                  </p>
                </li>
              </ul>
            </li>
            <li class="process-primary-item">
              <ul class="process-secondary-list list-unstyled">
                <li class="process-secondary-item">
                  <figure class="process-secondary-image">
                    <img class="w-100" src="<?php echo get_stylesheet_directory_uri();?>/public/assets/icons/process-registrations.svg" alt="">
                  </figure>
                  <h3 class="process-secondary-title">Inscrições</h3>
                  <p class="process-secondary-text">As inscrições serão feitas através do site da toodoo.</p>
                </li>
                <li class="process-secondary-item">
                  <figure class="process-secondary-image">
                    <img class="w-100" src="<?php echo get_stylesheet_directory_uri();?>/public/assets/icons/process-participate.svg" alt="">
                  </figure>
                  <h3 class="process-secondary-title">Quem pode participar?</h3>
                  <p class="process-secondary-text">
                    Todas as pessoas acima de 18 anos com muita vontade de aprender e se tornarem dev’s.
                  </p>
                </li>
                <li class="process-secondary-item">
                  <figure class="process-secondary-image">
                    <img class="w-100" src="<?php echo get_stylesheet_directory_uri();?>/public/assets/icons/process-exam.svg" alt="">
                  </figure>
                  <h3 class="process-secondary-title">Prova técnica</h3>
                  <p class="process-secondary-text">
                    O prazo para responder a prova técnica será sempre de 24h.s
                  </p>
                </li>
              </ul>
            </li>
          </ul>
        </div>
      </div>
    </section>
    <section class="section-depositions light-orbs">
      <div class="container">
        <div class="text-white content-container">
          <div class="depositions-info">
            <h3 class="depositions-title">
              Depoimentos de <span class="color-magenta">alunos</span>
            </h3>
            <p class="depositions-text fs-title-lg">
              Saiba o que os nossos alunos acharam sobre o programa!
            </p>
            <a class="toodoo-button ghost fs-title-lg text-white text-decoration-none text-uppercase" href="#">
              Clique e interaja
            </a>
          </div>
          <div class="students-depositions">
            <div class="student-info">
              <div class="student-balloon">
                <div class="student-information-container">
                  <h3 class="fs-title-lg fw-bold">Emanuel (Swift – IOS / Mobile)</h3>
                  <p class="student-text mb-1">
                    Olá, sou o Emanuel Motta, da stack de SWIFT iOS e vim aqui falar um pouco de como está sendo a minha experiência no programa BlastOff, que é um oferecimento TooDoo.
                    O programa tem sido muito legal, em todos os quesitos, gosto bastante dos métodos que são utilizados para nos
                    Olá, sou o Emanuel Motta, da stack de SWIFT iOS e vim aqui falar um pouco de como está sendo a minha experiência no programa BlastOff, que é um oferecimento TooDoo.
                    O programa tem sido muito legal, em todos os quesitos, gosto bastante dos métodos que são utilizados para nos
                  </p>
                </div>
              </div>
              <div class="student-balloon-detail"></div>
            </div>
            <figure class="student-image">
              <img class="w-100" src="<?php echo get_stylesheet_directory_uri();?>/public/assets/images/stundent-example.png" alt="">
            </figure>
          </div>
        </div>
      </div>
      <div class="list-container">
        <ul class="students-list list-unstyled">
          <li class="students-list-item">
            <figure>
              <img class="w-100" src="<?php echo get_stylesheet_directory_uri();?>/public/assets/images/stundent-example.png" alt="">
            </figure>
          </li>
          <li class="students-list-item">
            <figure>
              <img class="w-100" src="<?php echo get_stylesheet_directory_uri();?>/public/assets/images/stundent-example.png" alt="">
            </figure>
          </li>
          <li class="students-list-item">
            <figure>
              <img class="w-100" src="<?php echo get_stylesheet_directory_uri();?>/public/assets/images/stundent-example.png" alt="">
            </figure>
          </li>
          <li class="students-list-item">
            <figure>
              <img class="w-100" src="<?php echo get_stylesheet_directory_uri();?>/public/assets/images/stundent-example.png" alt="">
            </figure>
          </li>
        </ul>
        <ul class="students-tip-list list-unstyled">
          <li class="text-white students-tip-item active"></li>
          <li class="text-white students-tip-item"></li>
          <li class="text-white students-tip-item"></li>
          <li class="text-white students-tip-item"></li>
        </ul>
      </div>
    </section>
    <section class="section-doubts light-orbs">
      <div class="container">
        <div class="text-center">
          <h2 class="fs-display-md text-white fw-medium m-0">
            Dúvidas
          </h2>
        </div>
        <ul class="accordion list-unstyled my-5" id="accordion-doubts">
          <li class="accordion-item fs-title-lg mb-3 text-white">
            <div class="accordion-header">
              <button class="accordion-button fs-title-lg fw-bold text-white collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse-doubts-0" aria-expanded="true" aria-controls="collapse-doubts-0">
                A formação é presencial ou online?
              </button>
            </div>
            <div id="collapse-doubts-0" class="accordion-collapse collapse mt-3" aria-labelledby="headingOne" data-bs-parent="#accordion-doubts">
              <div class="accordion-body bg-blur">
                <p class="m-0">
                  Não, a formação é <span class="color-light-blue">100% online</span>, assim o colaborador poderá se desenvolver de onde estiver, com acompanhamentos individualizados e suporte a dúvidas a todo momento.
                </p>
              </div>
            </div>
          </li>
          <li class="accordion-item fs-title-lg mb-3 text-white">
            <div class="accordion-header">
              <button class="accordion-button fs-title-lg fw-bold text-white collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse-doubts-1" aria-expanded="true" aria-controls="collapse-doubts-1">
                Quais stacks desenvolvemos?
              </button>
            </div>
            <div id="collapse-doubts-1" class="accordion-collapse collapse mt-3" aria-labelledby="headingOne" data-bs-parent="#accordion-doubts">
              <div class="accordion-body bg-blur">
                <p class="m-0">
                  Não, a formação é <span class="color-light-blue">100% online</span>, assim o colaborador poderá se desenvolver de onde estiver, com acompanhamentos individualizados e suporte a dúvidas a todo momento.
                </p>
              </div>
            </div>
          </li>
          <li class="accordion-item fs-title-lg mb-3 text-white">
            <div class="accordion-header">
              <button class="accordion-button fs-title-lg fw-bold text-white collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse-doubts-2" aria-expanded="true" aria-controls="collapse-doubts-2">
                Quanto tempo de duração?
              </button>
            </div>
            <div id="collapse-doubts-2" class="accordion-collapse collapse mt-3" aria-labelledby="headingOne" data-bs-parent="#accordion-doubts">
              <div class="accordion-body bg-blur">
                <p class="m-0">
                  Não, a formação é <span class="color-light-blue">100% online</span>, assim o colaborador poderá se desenvolver de onde estiver, com acompanhamentos individualizados e suporte a dúvidas a todo momento.
                </p>
              </div>
            </div>
          </li>
          <li class="accordion-item fs-title-lg mb-3 text-white">
            <div class="accordion-header">
              <button class="accordion-button fs-title-lg fw-bold text-white collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse-doubts-3" aria-expanded="true" aria-controls="collapse-doubts-3">
                Lorem ipsum dolot do sit amet?
              </button>
            </div>
            <div id="collapse-doubts-3" class="accordion-collapse collapse mt-3" aria-labelledby="headingOne" data-bs-parent="#accordion-doubts">
              <div class="accordion-body bg-blur">
                <p class="m-0">
                  Não, a formação é <span class="color-light-blue">100% online</span>, assim o colaborador poderá se desenvolver de onde estiver, com acompanhamentos individualizados e suporte a dúvidas a todo momento.
                </p>
              </div>
            </div>
          </li>
        </ul>
        <div class="text-center text-white fs-title-lg">
          <figure class="m-0">
            <img src="<?php echo get_stylesheet_directory_uri();?>/public/assets/icons/rocket-icon.svg" alt="">
          </figure>
          <p class="my-4 mb-sm-5">
            Ainda possui dúvidas? Entre <br> em contato com a gente!
          </p>
          <a class="toodoo-button text-decoration-none" href="#">Fale conosco</a>
        </div>
      </div>
    </section>
    <section class="section-participate">
      <div class="container">
        <div class="participate-card light-orbs fs-headline-lg bg-blur text-white text-center">
          <p class="color-light-blue fw-bold mb-2">Faça parte</p>
          <p class="">
            Se você quer avançar no mundo da programação e não sabe por onde começar, o BlastOff é para você!
          </p>
        </div>
      </div>
    </section>
  </main>
<? get_footer();?>